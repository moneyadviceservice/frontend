...
RSpec.describe HTMLProcessor::AmpImg do
  ...
  describe '.process' do
    ...
    let(:amp_img_attributes) { amp_img_tag.attributes.transform_values(&:value) }
    ...
    it 'the parent container has the class amp-img-container' do
      expect(parent_container.attribute('class').value).to include('amp-img-container')
    end
    ...
    it 'copies the attributes across correctly from the original img' do
      expect(amp_img_attributes).to include(
        'srcset' => 'test_srcset',
        'src' => 'test_src',
        'sizes' => 'test_sizes',
        'alt' => 'test_alt',
    ...
    context 'when attributes are missing' do
      ...
      it 'does not create those attributes on the amp-iframe' do
        expect(amp_img_attributes.keys).not_to include(
