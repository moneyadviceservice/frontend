class AddEncryptedFields < ActiveRecord::Migration
  def change
    ...
    # rename old columns
    if column_exists? :users, :email, :email_old 
      rename_column :users, :email, :email_old
    end
    if column_exists? :users, :first_name, :first_name_old
      rename_column :users, :first_name, :first_name_old
    end
    if column_exists? :users, :last_name, :last_name_old
      rename_column :users, :last_name, :last_name_old
    end
    if column_exists? :users, :post_code, :post_code_old
      rename_column :users, :post_code, :post_code_old
    end
    if column_exists? :users, :contact_number, :contact_number_old
      rename_column :users, :contact_number, :contact_number_old
    end
    if column_exists? :users, :age_range, :age_range_old
