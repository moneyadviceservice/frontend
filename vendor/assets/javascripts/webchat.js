...
function woAfterLoad() {
  ...
  function getImg() {
    var rTimer = 6000;
    var pUrl = iWOGateway + "/poll.gif?d=dev.moneyadviceservice.org.uk&stamp=" + (new Date()).getTime();
    if (iWOSession != "") {
      pUrl += "&u=" + iWOSession
    }
    imgPoll.src = pUrl;
...
function inviteSound() {
  ...
  if (t != null) {
    t.src = iWOGateway + "/" + playSound + "?domain=dev.moneyadviceservice.org.uk";
...
function startInvite() {
  ...
  var ofsY;
  var iUrl = iWOGateway + "/invite.gif?d=dev.moneyadviceservice.org.uk&stamp=" + (new Date()).getTime();
...
function startChat(event) {
  if (event.clientX < locX + imgW - closeX || event.clientY > locY + closeY) {
    this.chatWindow = window.open("https://webchat.moneyadviceservice.org.uk/chat/chatstart.htm?domain=dev.moneyadviceservice.org.uk", "Chat", "width=484,height=361");
...
function getParams() {
  ...
  ifme.src = iWOGateway + "/params.js?d=dev.moneyadviceservice.org.uk&stamp=" + (new Date()).getTime() + ((iWOSession != "") ? "&u=" + iWOSession : "");
