define(['jquery', 'common'], function($, MAS) {
  ...
  var _getTarget = function($el, opts) {
    switch (opts.targetType) {
      case 'class':
        var $mmediateSibling = $el.next(opts.targetEl),
            targetIsImmediateSibling = !!$mmediateSibling.length;
        return (targetIsImmediateSibling)? $mmediateSibling: $el.siblings(opts.targetEl);
      case 'href':
