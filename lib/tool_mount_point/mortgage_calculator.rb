  ...
  class MortgageCalculator < Base
    ADDITIONAL_TOOLS = {
      'cy' => {
        ...
        'land-and-buildings-transaction-tax-calculator-scotland' =>
          'cyfrifiannell-treth-trafodion-tir-ac-adeiladau-alban',
        'land-transaction-tax-calculator-wales' =>
          'cyfrifiannell-treth-trafodiadau-tir-cymru'
      },
      'en' => {
    ...
    def alternate_url(url, locale = nil)
      ...
      ADDITIONAL_TOOLS[locale].keys.each do |calculator_name|
        if url.include?(calculator_name)
          return url.gsub(
            calculator_name,
            translated_calculator_name(locale, calculator_name)
          )
        end
      end
      url
