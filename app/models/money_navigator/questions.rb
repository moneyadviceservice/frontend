module MoneyNavigator
  ...
  class Questions
    ...
    def results
      ...
      sections_array = MoneyNavigator::ContentRules.all.each_with_object([]) do |section_rules, sections_array_accumulator|
        heading_content_array = section_rules[:heading_rules].each_with_object([]) do |heading_rule, heading_content|
        ...
        next if heading_content_array.empty?
    ...
    def to_hash
      answers_hash = QUESTIONS.each_with_object(HashWithIndifferentAccess.new) do |question, accumulator_hash|
        accumulator_hash[question[:code]] = send((question[:code]).to_s)
      end
    ...
    def normalise_answers_hash(answers_hash)
      ...
      normalised_answers_hash = QUESTIONS.each_with_object(HashWithIndifferentAccess.new) do |q_hash, hash|
        ...
        ans_nil_or_empty_arr = answers_hash[q_hash[:code]].blank?
    ...
    def obtain_content_for_heading(heading_rule, answers_hash)
      content = heading_rule[:content_rules].each_with_object({}) do |content_rule, content_accumulator|
    ...
    def convert_hash_to_flags(question_answers_hash)
      ...
      qn_flags_hash[:all] = ''
      question_flags_hash = QUESTIONS.each_with_object(qn_flags_hash) do |qn, acc_qn_flags_hash|
    ...
    def unflatten_answers_array(question_answers_array)
      ...
      unflattened = question_answers_array.each_with_object(HashWithIndifferentAccess.new) do |qa_str, qa_hash|
    ...
    def flatten_answers_hash(answers_hash)
      ...
      flattened = answers_hash.each_with_object([]) do |q_code, arr, answers|
        answers.each { |answer| arr << "#{q_code}_#{answer}" }
      end
