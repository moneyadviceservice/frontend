...
task csslint: :environment do
  ...
  Rails.application.assets.instance_variable_get(
    '@postprocessors'
  )['text/css'].delete(AssetPipeline::Processors::CssLint)
  ...
  Rails.application.assets.each_file do |pathname|
    next unless pathname.basename.to_s =~ /\A[^_][a-zA-Z0-9_.]+.scss\z/
    ...
    if errors.present?
      ...
      errors.each do |error|
        message = "message: #{error['message']}: #{error['evidence']}"
        puts "  line: #{error['line']}, col: #{error['col']}, #{message}"
